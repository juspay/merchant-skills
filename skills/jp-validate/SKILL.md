...
This is **action-oriented**: write and run real tests, but **checkpoint before risky/external actions** (starting the dev server, running live transactions, anything that moves money or hits production). Default to the production environment unless told sandbox; a dummy/test gateway returned by the MCP is acceptable for integration testing.
